...
import { getTrainingSession, edit } from '~/api/admin/training_session'
...
export default {
    ...
    async asyncData ({app, params, query}) {
        const {data: training_session} = await getTrainingSession(app.$axios, params.id)
    ...
    computed: {
        totalTime () {
            return this.caculateCalories(this.training_session.exercises).time
        },

        calories () {
            return this.caculateCalories(this.training_session.exercises).calo
        }
    },
    ...
    methods: {
        ...
        pushExercise (exercises, dialog) {
            this.training_session.exercises.push(...exercises)
        ...
        async submitForm () {
            const form = {
                ...
                desc: this.training_session.desc,
                exercises: this.training_session.exercises
            }
            try {
                await edit(this.$axios, this.$route.params.id, form)
